...
// 1. for each browserView: captures and shows thumbnail
// 2. zoom out
// 3. for each browserView hide thumbnail
...
global.win = null;
global.browserViews = Array(APPS.length);

function showBrowserViews() {
...
function finishZoomIn(view) {
  ...
  setTimeout(showBrowserViews, 10000)
}
...
function createWindow () {
  ...
  const addWebview = function (scrollContent, url, i) {
    ...
    const browserView = new BrowserView({"webPreferences": {"show": false}});
    browserViews[i] = browserView;
...
function captureScreenshotForApp(index) {
  ...
  offscreenWindow.webContents.on('did-finish-load', async() => {
    ...
    let nativeImage = await offscreenWindow.webContents.captureScreenshot().then(image => {
      ...
      browserViews[index].hide(true, image);

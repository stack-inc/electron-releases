...
function createWindow () {
  ...
  const contentScroll = new BaseView();
  scrollContent.addChildView(contentScroll);
  contentScroll.setBounds({ x: 0, y: 0, width: APPS.length * (APP_WIDTH + GAP), height: 600 });

  // Webview
  const addWebview = function (contentScroll, url, i) {
    ...
    contentScroll.addChildView(webContentView);
  };

  var i = 0;
  APPS.forEach((app) => {
    addWebview(contentScroll, app, i++);
  });
